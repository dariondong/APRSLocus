...
### 📱 桌面小组件（Android）
- **4 列 × 2 行**：一眼看到当前天气与**此刻该注意什么**
- 第 1 行：天气（图标 / 温度 / 天气现象 / 今日高低温）+ 3 个指标格（湿度、气压、风力、能见度、降水量、露点……按当前天气自动挑选最要紧的三项）
- 第 2 行：**业余无线电操作提示** ×4（按级别排序：安全警示 → 注意 → 通联机会 → 操作提示，危险级红底高亮），与 App 内天气面板同一套判定规则
- 背景渐变随天气与系统深色模式变化，配色与天气面板同源
