...
- **语言选择器**：显示设置 → 通用新增语言切换
- **主要界面本地化**：主页导航、设置入口、设置标题、搜索框等核心文案随语言切换

### 🐛 修复
- **深色模式路由过渡闪白**：MaterialApp 补全 canvasColor/surface，主题变化时重建
- **主题色启动应用**：_loadPrefs 完成后立即应用保存的主题

## [1.4.3] - 2026-08-29

### 🐛 修复
- **重启后皮肤不生效**：显示设置（深色/自定义主题色）加载完成后未应用，启动时仍显示默认皮肤；改为 `_loadPrefs` 完成后立即应用保存的主题
- **深色模式启动闪白**：SplashPage 背景依赖浅色 C.bg，深色模式下启动瞬间白底；改为固定深色品牌背景，深浅模式统一
- **台站变多严重卡顿**：台站列表每个 tile 的入场动画在每次 60ms 通知时全部重播；改为按台站稳定 key 只动画一次 + 列表结果缓存（台站版本/筛选参数未变时复用）+ 呼号校验正则缓存 + 收包通知节流提升至 250ms
