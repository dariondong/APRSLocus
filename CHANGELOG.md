...
### ✨ 新增
- **地图群组跟踪**：把关心的呼号编成「跟踪组」在整屏地图上持续跟踪，支持横屏导航风格（左成员栏 + 大地图）；成员状态 / 速度 / 距离 / 轨迹一览，点击成员锁定跟随、一键全览
  - 入口：地图页右上角新增👥群组跟踪按钮
  - 可新建 / 编辑 / 删除多个跟踪组，自由勾选台站组成（自动包含自己）
  - 成员在线 / 移动状态、实时距离、轨迹显示；跟随成员移动自动居中
  - 横屏显示左侧成员栏，竖屏显示底部成员条，均可上下翻
