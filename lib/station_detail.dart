...
class StationDetail extends StatefulWidget {
  ...
  const StationDetail(
      {super.key, required this.state, required this.station});
  @override
...
class _StationDetailState extends State<StationDetail> {
  ...
  Widget build(BuildContext context) {
    ...
    // 同基础呼号的相关台站（含不同 SSID）
    final related = widget.state.stations
            ...
            children: [
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  if (related.isNotEmpty) ...[
                    ...
                    SoftCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            Icon(Icons.link_rounded, size: 15, color: C.cyan),
                            SizedBox(width: 6),
                            Text('${S.of(context).relatedStations} (${related.length})',
                  ...
                  Row(children: [
                    Expanded(
                        child: _metric(S.of(context).speedLabel, s.speedStr, Icons.speed_rounded, C.blue)),
                    SizedBox(width: 10),
                    Expanded(
                        child:
                            _metric(S.of(context).altitude, s.altStr, Icons.height_rounded, C.purple)),
                    SizedBox(width: 10),
                    Expanded(
                        child: _metric(S.of(context).courseLabel,
                            s.course != null
                                ? '${s.course!.toStringAsFixed(0)}°'
                                : '--',
                            Icons.navigation_rounded, C.cyan)),
                  ]),
                  const SizedBox(height: 12),
                  // 快捷操作（Wrap 自动换行，窄屏不溢出）
                  Wrap(spacing: 8, runSpacing: 8, children: [
                    _action(Icons.content_copy_rounded, S.of(context).copyCoords, () {
                      Clipboard.setData(ClipboardData(text: s.posStr));
                      _toast('已复制坐标：${s.posStr}');
                    }),
                    _action(Icons.grid_4x4_rounded, S.of(context).copyGrid, () {
                      Clipboard.setData(ClipboardData(text: s.grid));
                      _toast('已复制网格：${s.grid}');
                    }),
                    _action(Icons.map_rounded, S.of(context).openInMap, () {
                      widget.state.focusOnMap(s);
                      Navigator.pop(context);
                    }),
                    _action(Icons.navigation_rounded, S.of(context).navigate, () {
                      _openNavigation(s);
                    }),
                  ]),
                  if (widget.state.myStation != null) ...[
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        Icon(Icons.navigation_rounded, color: C.blue, size: 18),
                        SizedBox(width: 8),
                        Text(
                          '距我 ${s.distKm(widget.state.myLat!, widget.state.myLng!).toStringAsFixed(1)}km'
                          ' · 方位 ${s.bearingFrom(widget.state.myLat!, widget.state.myLng!).toStringAsFixed(0)}°',
                  ...
                  if (s.wx != null) ...[
                    ...
                    Container(
                      ...
                      child: Row(children: [
                        Icon(Icons.cloud_rounded, color: C.cyan, size: 20),
                        SizedBox(width: 10),
                        Expanded(
                          child: Text('气象数据 · ${s.wx}',
                  ...
                  SoftCard(
                    ...
                    child: Column(children: [
                      KV(S.of(context).grid, s.grid, icon: Icons.grid_4x4_rounded),
                      SizedBox(height: 8),
                      KV(S.of(context).latitude, s.lat.toStringAsFixed(5), icon: Icons.explore_rounded),
                      SizedBox(height: 8),
                      KV(S.of(context).longitude, s.lng.toStringAsFixed(5), icon: Icons.explore_rounded),
                      SizedBox(height: 8),
                      KV(S.of(context).lastSeen, s.lastSeen, icon: Icons.access_time_rounded),
                      SizedBox(height: 8),
                      KV('符号', '${s.symbol}  ${s.typeName}',
                  ...
                  if (s.path != null && s.path!.isNotEmpty) ...[
                    ...
                    SoftCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            Icon(Icons.route_rounded, size: 14, color: C.orange),
                            SizedBox(width: 6),
                            Text(S.of(context).forwardingPath, style: ts(12, c: C.orange, w: FontWeight.w700)),
                          ]),
                          SizedBox(height: 10),
                          Wrap(
                            spacing: 2,
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            ...
                            SizedBox(width: 8),
                            Text(S.of(context).fmoInfo,
                                style: ts(13,
                                    c: C.green, w: FontWeight.w700)),
                            Spacer(),
                            GestureDetector(
                              onTap: () {
                              ...
                              child: Icon(Icons.copy_rounded,
                                  color: C.grey, size: 16),
                            ),
                          ]),
                          SizedBox(height: 10),
                          ...s.fmo!.entries.map((e) => Padding(
                    ...
                    SoftCard(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(children: [
                            Container(
                              ...
                              child: Icon(Icons.terminal_rounded,
                                  color: C.blue, size: 15),
                            ),
                            SizedBox(width: 8),
                            Text(S.of(context).aprslocusInfo,
                                style: ts(13,
                                    c: C.blue, w: FontWeight.w700)),
                            Spacer(),
                            GestureDetector(
                              onTap: () {
                                final txt = (s.aprslocus ??
                                        _aprslocusFromComment(s))
                                    .entries
                                    .map((e) => '${e.key}: ${e.value}')
                                    .join('\n');
                                Clipboard.setData(ClipboardData(text: txt));
                                _toast('已复制 APRSlocus 信息');
                              },
                              child: Icon(Icons.copy_rounded,
                                  color: C.grey, size: 16),
                            ),
                          ...
                          SizedBox(height: 10),
                          ..._aprslocusEntries(s).entries.map((e) => Padding(
                                padding: const EdgeInsets.only(bottom: 6),
                                child: Row(children: [
                                  Text(e.key,
                                      style: ts(11, c: C.slate)),
                  ...
                  SoftCard(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(children: [
                          Icon(Icons.route_rounded, size: 16, color: C.slate),
                          SizedBox(width: 6),
                          Text('轨迹 (${s.track.length} 点)',
                  ...
                  // 最近数据包
                  if (pkt.isNotEmpty) ...[
                    Text(S.of(context).recentPackets, style: ts(13, w: FontWeight.w600)),
                    SizedBox(height: 8),
                    ...pkt.map((p) => Container(
                    ...
                    GestureDetector(
                      onTap: () {
                        if (_msg.text.isNotEmpty) {
                          widget.state.sendMessage(s.call, _msg.text.trim());
                          _msg.clear();
  ...
  Future<void> _openNavigation(Station s) async {
    ...
    Future<bool> tryLaunch(Uri uri, {LaunchMode mode = LaunchMode.externalApplication}) async {
      try {
    ...
    // 2. 系统地图
    final geoUri = Uri.parse('geo:${s.lat},${s.lng}?q=${s.lat},${s.lng}($name)');
  ...
  List<({String call, bool isClickable})> _pathHops(String path) {
    ...
    for (final raw in path.split(',')) {
      ...
      const skip = {
        'WIDE', 'TRACE', 'RELAY', 'TCPIP', 'APRS', 'NOGATE', 'SAT', 'ARISS',
        'T2CS', 'T2HUB', 'T2PHX', 'T2NZ', 'T2EUROPE', 'T2ANZ',
      };
      final isT2Node = base0.startsWith('T2');
      final isQConf = qm != null || base0.startsWith('QA');
      // 纯数字 / 单字符路径段也视为标识
      final isNumeric = RegExp(r'^\d+$').hasMatch(base0);
      final isClickable = !skip.contains(base0) &&
          !isT2Node && !isQConf && !isNumeric &&
    ...
    for (var i = 0; i < hops.length; i++) {
      if (i > 0) {
        widgets.add(Padding(
          ...
          child: Icon(Icons.arrow_forward_rounded, size: 12, color: C.greyLight),
        ));
      }
      final hop = hops[i];
      if (hop.isClickable) {
        widgets.add(GestureDetector(
          ...
          child: Container(
            ...
            child: Text(hop.call, style: ts(11, c: C.orange, w: FontWeight.w700)),
          ),
        ));
      } else {
        widgets.add(Container(
          ...
          child: Text(hop.call, style: ts(10, c: C.grey, w: FontWeight.w500)),
        ));
      }
    }
    return widgets;
  ...
  Widget _metric(String label, String value, IconData icon, Color c) {
    return SoftCard(
      ...
      child: Column(children: [
        ...
        SizedBox(height: 6),
        Text(value, style: ts(13, c: c, w: FontWeight.w700)),
        Text(label, style: ts(9, c: C.grey, w: FontWeight.w600, ls: 0.8)),
      ]),
    );
  }
}
...
class _TrackPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(map(pts.first.lat, pts.first.lng), 3, Paint()..color = C.greyLight);
    canvas.drawCircle(map(pts.last.lat, pts.last.lng), 4, Paint()..color = color);
  }
