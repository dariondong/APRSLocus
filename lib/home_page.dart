...
class _HomePageState extends State<HomePage> {
    ...
    widget.state.onNewMessage = (src, text, groupId) {
      ...
      if (groupId != null) {
        for (final g in widget.state.chatGroups) {
          if (g.id == groupId) { groupName = g.name; break; }
        }
      }
      _showBubble = true;
      _bubbleCall = groupName != null ? '群·$groupName' : src;
      _bubbleText = text;
      if (mounted) setState(() {});
      _bubbleTimer?.cancel();
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          SizedBox(width: 10),
          Expanded(child: Text('群聊邀请', style: ts(16, w: FontWeight.w700))),
        ]),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text('$from 邀请你加入群聊', style: ts(13)),
            SizedBox(height: 8),
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('群名: $groupName', style: ts(13, w: FontWeight.w600)),
                  SizedBox(height: 4),
                  Text('群呼号: $groupCall', style: ts(12, c: C.orange, w: FontWeight.w600)),
                ],
              ),
            ),
          ],
        ...
        actions: [
          TextButton(
            onPressed: () {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('已接受邀请，加入 $groupName'),
                  backgroundColor: C.green,
                  duration: const Duration(seconds: 2),
                ),
              );
            },
            child: Text('接受', style: ts(14, c: C.green, w: FontWeight.w700)),
          ),
          TextButton(
            onPressed: () {
              Navigator.pop(ctx);
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(
                  content: Text('已拒绝 $groupName 的邀请'),
                  duration: const Duration(seconds: 2),
                ),
              );
            },
            child: Text('拒绝', style: ts(14, c: C.red)),
          ),
        ],
      ),
    );
  ...
  List<(IconData, IconData, String)> get _nav {
    ...
    return [
      ...
      (Icons.chat_bubble_outline_rounded, Icons.chat_bubble_rounded, s.messages),
            ...
            Expanded(
              child: Column(children: [
                ...
                Expanded(child: ListenableBuilder(
                    listenable: widget.state, builder: (_, _) => _page())),
                _connBanner(),
              ]),
            ),
          ]),
        ...
        Padding(
          ...
          child: Row(children: [
            ...
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('APRSlocus',
                  style: ts(compact ? 14 : 17,
                      w: FontWeight.w800, ls: -0.3)),
              if (!compact) ...[
                SizedBox(height: 1),
                Text('APRS 定位追踪', style: ts(10, c: C.grey)),
              ],
            ]),
          ]),
        ),
        ...
        Expanded(
          child: ListView.separated(
            padding: EdgeInsets.symmetric(
                horizontal: compact ? 8 : 12, vertical: 4),
  ...
  Widget _myPanel() {
    ...
    final locColor = fix ? C.green : widget.state.loc.running ? C.blue : C.yellow;
    final connColor =
        widget.state.connected ? C.green : widget.state.connecting ? C.blue : C.slate;
    return Container(
      ...
      child: Column(children: [
        ...
        Row(children: [
          ...
          Expanded(
            child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text(widget.state.myCall, style: ts(13, w: FontWeight.w700)),
        ...
        Row(children: [
          _dot(locColor),
          SizedBox(width: 6),
          Text(widget.state.locStatus, style: ts(11, c: locColor, w: FontWeight.w600)),
          Spacer(),
          _dot(connColor),
          SizedBox(width: 6),
          Text(widget.state.connected ? '已连接' : '演示',
              style: ts(11,
        ...
        Row(children: [
          ...
          SizedBox(width: 4),
          Text('下次上报 ${widget.state.nextBeaconIn}',
              style: ts(10, c: C.slate)),
          Spacer(),
          Icon(Icons.sync_rounded, size: 12, color: C.grey),
          SizedBox(width: 4),
          Text('信标 ${widget.state.beaconsSent} 次',
              style: ts(10, c: C.slate)),
        ]),
        ...
        Row(children: [
          Expanded(
            child: OutlinedButton.icon(
              onPressed: () {
                if (widget.state.myHasFix) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(
                      content: Text(
                        '位置已上报 · 网格 ${widget.state.myGrid}'
                        '${widget.state.connected ? ' · 已发往 APRS-IS' : ' · 演示'}'),
              ...
              label: Text(widget.state.myHasFix ? '立即上报' : '获取定位'),
          ...
          Expanded(
            child: OutlinedButton.icon(
              ...
              label: Text(widget.state.connected
                  ? '断开连接'
              ...
              style: OutlinedButton.styleFrom(
                foregroundColor:
                    widget.state.connected ? C.red : C.blue,
                side: BorderSide(
        ...
        builder: (context, constraints) {
          ...
          return Row(children: [
            ...
            if (wide) ...[
              ...
              SizedBox(width: 12),
              _statTag('${widget.state.online}', '在线', C.green, C.greenBg),
              SizedBox(width: 8),
              _statTag('${widget.state.moving}', '移动', C.blue, C.blueBg),
              SizedBox(width: 8),
              _statTag('${widget.state.packetsRx}', '收包', C.slate, C.greyBg),
            ] else
              _statTag('${widget.state.online}', '在线', C.green, C.greenBg),
          ]);
        },
  ...
  Widget _statTag(String val, String label, Color c, Color bg) {
    return Container(
      ...
      child: Text('$val $label', style: ts(11, c: c, w: FontWeight.w600)),
    );
  }
          ...
          child: Row(children: [
            ...
            Expanded(
              child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text(connecting ? '正在连接服务器…' : '未连接 APRS-IS 服务器',
                        style: ts(13, c: Colors.white, w: FontWeight.w700)),
                    Text(
                      connecting
                          ? '正在连接 ${st.aprs.server}:${st.aprs.port}…'
                          : '连接后可接收附近台站定位与消息',
                      style: ts(10,
              ...
              child: GestureDetector(
                ...
                child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  Stack(clipBehavior: Clip.none, children: [
                    Icon(sel ? _nav[i].$2 : _nav[i].$1,
                        color: sel ? C.blue : C.grey, size: 22),
